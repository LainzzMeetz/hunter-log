...
function PenaltyWindow({ player, setPlayer, setIsInPenalty }) {
  ...
  useEffect(() => {
    axios.get('https://hunter-log.onrender.com/api/quests?type=penalty')
      .then(res => {
  ...
  const handleCompletePenalty = async () => {
    try {
      ...
      await axios.put(`https://hunter-log.onrender.com/api/quests/${penaltyQuest._id}/complete`);
      
      // 2. Tell the "System" the penalty is over
      const res = await axios.post('https://hunter-log.onrender.com/api/penalty/complete');
