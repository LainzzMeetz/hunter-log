...
@app.get("/")
async def root():
    return {"message": "Hunter's Log: [System Core v14.5 Online]"}
...
@app.get("/api/quests/{quest_id}", response_model=Quest):
    async def get_quest(quest_id: PydanticObjectId): quest = await Quest.get(quest_id); if not quest: raise HTTPException(status_code=404, detail="Quest not found"); return quest

# --- CRITICAL FIX 1: UPGRADED SUB-TASK ENDPOINT ---
# It now returns the updated PLAYER object, not the Quest
...
async def toggle_sub_task(quest_id: PydanticObjectId, sub_task_title: str):
    ...
    # If quest is already done, just return the player
    if quest.completed:
    ...
    if all_tasks_complete:
        print(f"All sub-tasks for '{quest.title}' complete. Granting reward.")
        quest.completed = True
        await quest.save()
        
        # --- GRANT REWARD LOGIC (MOVED HERE) ---
        ...
        # If tasks are NOT all complete, just save the quest state
        await quest.save()
        # And return the player's current (unchanged) state
        return await get_player_instance()

# --- CRITICAL FIX 2: UPGRADED COMPLETE QUEST ENDPOINT ---
...
async def complete_quest(quest_id: PydanticObjectId):
    ...
    # This endpoint is NOW ONLY for quests WITHOUT sub-tasks (like timers)
    if quest.sub_tasks:
        raise HTTPException(status_code=400, detail="This quest must be completed via its sub-tasks.")
        
    quest.completed = True
    await quest.save()
    player = await get_player_instance()
        
    # (Reward logic is unchanged for timer quests)
    ...
    leveled_up = False
    while player.exp >= player.exp_to_next_level:
        player.level += 1; player.exp -= player.exp_to_next_level
        player.exp_to_next_level = int(player.exp_to_next_level * 1.15); leveled_up = True
    await player.save()
    return player
