...
# --- NEW: Seeder Logic ---
# We are moving the seed.py logic directly into main.py
async def seed_database_logic():
    print("DATABASE SEEDING v14.8 INITIATED...")
    ...
    await Quest(
        ...
        exp_grant=10, rank="D", stat_reward="focus", stat_points=1, duration_minutes=5
    ).insert()
    await Quest(
        title="Workout", description="15–45 min exercise.", type="daily",
        exp_grant=20, rank="C", stat_reward="strength", stat_points=1, duration_minutes=45
    ).insert()
    await Quest(
    ...
    print("--- DATABASE SEEDING v14.8 COMPLETE ---")
    return {"message": "System has been reset to v14.8 defaults."}
# --- End of Seeder Logic ---
...
@app.get("/")
async def root():
    return {"message": "Hunter's Log: [System Core v14.8 Online]"} # Version Up!

# --- NEW: "DEVELOPER BACKDOOR" RESET ENDPOINT ---
@app.get("/api/system/reset")
async def system_reset():
    """
    DANGER: Wipes the entire database and re-seeds it.
    This is the "System Reset" you need.
    """
